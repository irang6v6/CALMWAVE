...
public class DataController {
    ...
    public ResponseEntity<?> getDoneWorkForToday(
    ...
    public ResponseEntity<?> getDoneWorkForDateRange(
        @RequestHeader(value = "AccessToken") String token,
        @PathVariable("start_date") String start_date, @PathVariable("end_date") String end_date) {
        User user = jwtUtil.getUser(token);
        List<DoneWorkDto> doneWorkDtos = dataService.findDoneWorkForDateRange(user.getId(),
            start_date, end_date);
        return ResponseEntity.ok().body(doneWorkDtos);
    }
    ...
    public ResponseEntity<?> getDoneWorkDatesForToday(
        @RequestHeader(value = "AccessToken") String token) {
        User user = jwtUtil.getUser(token);
    ...
    public ResponseEntity<?> getDoneWorkDatesForDateRange(
        @RequestHeader(value = "AccessToken") String token,
        @PathVariable("start_date") String start_date, @PathVariable("end_date") String end_date) {
        User user = jwtUtil.getUser(token);
        List<DoneWorkDatesDto> doneWorkDatesDtos = dataService.findDoneWorkDatesForDateRange(
            user.getId(), start_date, end_date);
    ...
    public ResponseEntity<?> getDoneBeforeAimWorksForRange(
        @RequestHeader(value = "AccessToken") String token,
        @PathVariable("start_date") String start_date, @PathVariable("end_date") String end_date) {
        User user = jwtUtil.getUser(token);
        Long doneBeforeAimWorksCnt = dataService.findDoneBeforeAimWorksForRange(user.getId(),
            start_date, end_date);
    ...
    public ResponseEntity<?> resultPage(@RequestHeader(value = "AccessToken") String token) {
        ...
        if (user != null) {
            ...
            resultMap.put("pieChartByCategory",
                workPeriodService.findWorkDurationByUserAndCategory(id));
            resultMap.put("radarChart",
                workPeriodService.findWorkDurationByUserIdForRadarChart(id));
            resultMap.put("numOfTotalWork",
                workService.getTodo(id).size() + workService.getDone(id).size());
            resultMap.put("numOfUnfinished", workService.getTodo(id).size());
            resultMap.put("numOfDone", workService.getDone(id).size());
            resultMap.put("numOfTurtle",
                postureService.countByUserAndPostureCName(user, PostureCName.turtle));
            resultMap.put("numOfTilted",
                postureService.countByUserAndPostureCName(user, PostureCName.tilted));
            resultMap.put("percentOfTodoAndDone", workService.getPercentOfTodoAndDone(id));
            resultMap.put("percentOfWorkTimeAndAimedTime",
                workService.getPercentOfTotalAndAimedTime(id));
            resultMap.put("schedulerData",
                workPeriodService.findByWorkInAndStartTimeAfter(workService.findAllByUser(id)));
